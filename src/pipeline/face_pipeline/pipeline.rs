use anyhow::Error;
use ndarray::{Array1};
use crate::pipeline::face_pipeline::config::{FaceAlignmentConfig, FaceDetectionConfig, FaceIdentificationConfig, FaceQualityConfig, FaceSelectionConfig};
...
fn match_face_quality(q: usize) -> FaceQualityClass {
    match q {
...
pub struct FaceExtractionResult{
    ...
    pub selected_face_box: Option<Array1<f32>>
}

impl FaceExtractionResult {
    fn new() -> FaceExtractionResult {
...
impl FacePipeline {
    ...
    pub async fn extract(&self, im_bytes: &[u8]) -> Result<FaceExtractionResult, Error> {

        let mut result = FaceExtractionResult::new();
        ...
        let (detections, key_points)  = match self.face_detection.call(&image, Some(self.debug)).await {
        ...
        let (selected_face_box, selected_face_point) = match self.face_selection.call(&image, detections, key_points, Some(self.enroll), Some(self.debug)) {
        ...
        if let _selected_face_box = selected_face_box {
            let aligned_face_image = match self.face_alignment.call(&image, _selected_face_box.clone(), selected_face_point, Some(self.debug)) {
            ...
            let (quality_score, quality_class) = match self.face_quality.call(aligned_img_arr, Some(self.debug)).await {
            ...
            let facial_feature = match self.face_extraction.call(aligned_img_arr, Some(self.debug)).await {
                Ok(facial_feature) => {facial_feature}
                Err(e) => {
                    return Err(Error::from(e))
                }
            };
            result.facial_feature = Some(facial_feature[0].to_owned().into_shape((facial_feature[0].len(),)).unwrap());
            result.face_count = face_count;
            result.face_quality = Some(match_face_quality(quality_class[0].to_owned()));
            result.quality_score = Some(quality_score[0]);
            result.selected_face_box = _selected_face_box;
        }
        if self.debug {
            println!("{:?}", result);
        }
...
#[cfg(test)]
mod tests {
    use crate::pipeline::face_pipeline::pipeline::FacePipeline;
    use crate::triton_client::client::TritonInferenceClient;
    ...
    async fn test_pipeline() {
        ...
        // };
        //
        // let pipeline = match FacePipeline::new(triton_host, triton_port, Some(true), Some(false)).await {
        ...
        // };
        //
        // pipeline.extract(im_bytes).await;
