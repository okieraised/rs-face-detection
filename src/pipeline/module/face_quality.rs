...
impl FaceQuality {
    ...
    pub async fn call(&self, imgs: &[Mat], is_debug: Option<bool>) -> Result<(Vec<f32>, Vec<usize>), Error>{
        ...
        for i in 0..batch_size {
            ...
            if debug {
                match imwrite("./bgr.png", &converted_img, &opencv::core::Vector::default()) {
                    Ok(_) => {}
                    Err(e) => return Err(Error::from(e))
                };
            }
            ...
            let mut predict = flattened_net_out.iter().enumerate().max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()).map(|(idx, _)| idx).unwrap();
